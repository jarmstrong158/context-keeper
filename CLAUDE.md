...
**Subagents get constraints only.** SessionStart does not fire for them and they do not inherit the parent's injected context. The `subagent_start.py` hook (SubagentStart) injects the constraints block so a spawned agent at least knows the rules — but not the decisions or pipelines, because that hook fires once per agent and a fan-out multiplies it. When dispatching a subagent for design work, tell it to call `get_project_summary` itself.
...
- **global_scope** — a constraint scoped `global` when the evidence points at a real path. Global scope excludes a rule from the drift check, the `.claude/rules/` projection and `scope_guard` all at once, leaving the session-start summary as its only delivery route — and that summary is the thing that truncates on a large store. Set the suggested scope with `update_entry`, or leave it global if the rule genuinely applies everywhere. Only flagged when a concrete path can be named, so an unflagged global constraint is not necessarily fine — just not obviously fixable.
