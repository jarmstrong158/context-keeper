...
| **Retrieval hit@5** | **86.7%** (13/15) | For ~87% of natural-language questions, the entry that answers it is in the top 5 returned. | `get_context` top-5 vs a labeled gold set (`retrieval_eval` scorer logic). |
...
### 2. Retrieval hit@k / MRR — `retrieval_eval.py` methodology
`run_eval.py` itself is **not runnable here** (needs sibling `llm-evals`). The
...
|---|---|---|
| `run_eval.py`, `retrieval_eval.py` | Sibling `../llm-evals` repo absent (`core.runner/baseline/scorers`) **and** real stores `Clark`/`Conductor`/`context-keeper` absent. | Runner can't import or find corpora. Methodology reproduced in `synthetic_corpus.py`. |
...
| `semantic.py` / `run_eval.py --semantic` | No local embedder — Ollama + `nomic-embed-text` not reachable (`localhost:11434` down). | Semantic blend cannot be exercised; the documented lexical→semantic hit@5 uplift (80%→93% on real data) is **not reproduced here** and is not claimed. |
